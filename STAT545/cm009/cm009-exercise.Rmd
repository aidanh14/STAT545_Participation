...
```{r}
(lotr_untidy <- bind_rows(lotr1, lotr2, lotr3)) # put brackets rounf line to print output
```
...
gather(lotr_untidy, key = "Gender", value = "Word", Female:Male)
gather(lotr_untidy, key = "Gender", value = "Word", contains("ale")) #selects data from all columns with "ale" in var name
```
...
```{r}
spread(lotr_tidy, key="Race", value="Words") #moves row values in "Race" up to columns
```
...
```{r}
##### From ?expand documentation
expand(mtcars, vs, cyl) #Returns all possible combinations ofvs and cyl
