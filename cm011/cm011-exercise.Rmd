...
a[1]
a[1][1]
"holla"[1] # note: you can't iterate through each char in a string like you can in other languages. "stringr" package can
paste("hi", "you")
v1 <- "hi"
v2 <- "you"
paste(v1, v2)
...
```{r}
a * 1:3 # so this will be a * [ 1 2 3 1 2 3 ...]
a * 1:2 # no warning b/c length(a) is divisible by length(1:2)
```
...
a < 5
a > 0 & a < 5
a > 0 && a < 5 # not vectorized, only the first item of each vector

# Use functions "all" or "any" to reduce a vector of non-1 length to 1
any(a > 0 & a < 5)
all(a > 0 & a < 5)
```
...
a
# watch out:
(a[1:n] <- 1:2) # for some reason the brackets make it ignore the [1:n] indexing
```
...
```{r}
setNames(course, c("subject", "code", "section")) # kind of like a dictionary in python

# note: the names don't change the vector at all, just "on top" of them
```
...
```{r}
names(course) <-  c("subject", "code", "section") # actually changes the names of the vector, doesn't just return a changed vector
course
names(course)
```
...
```{r}
course <- c(subject="STAT", code="545A", section="S01") # metaprogramming b/c you don't need to specify names are strings
course
```
...
course[["section"]] # just returns value without the name

# note: it'll only return the first value if two vals have the same name
foo <- c(a=5, a=6, b=7)
foo["a"]
foo[["a"]]
```
...
```{r}
(course <- list("STAT", 545)) # two entries, each vectors of length 1
```
...
```{r}
(my_list <- list(year=2018, instructor=c("Vincenzo", "Coia"), fav_fun=typeof)) # even can hold functions
```
...
```{r}
my_list[2]
my_list[[2]] # sheds the list structure, just the contents
```
...
my_list["year"]
my_list[["year"]]
my_list$year # same thing as double square brackets, sheds the list structure
```

Smells a little like `data.frame`s. It turns out a `data.frame` is a special type of list:

```{r}
# so a data frame is a special type of list, where each component is a named vector and are all the same length
...
# matrix:
diag(5) # all components have to be the same type in a matrix
```
