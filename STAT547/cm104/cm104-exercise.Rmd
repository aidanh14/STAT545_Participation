...
```{r}
comm_fun <- function(name, commute)
  str_c(name, " takes ", commute, " minutes to get to work.")
...
```{r}
map2_chr(name, commute, function(name, commute) {
  str_c(name, " takes ", commute, " minutes to get to work.")})
```
...
```{r}
## NOTE: map2() functions always expect .x and .y or ..1 and ..2 to reference to two input vectors.
...
```{r}
got_alias # There's a NULL list in here, not gonna work with unnest()
got_alias %>%
  drop_na() %>% # Now we'v dropped the NULL list and it'll work
  unnest()
```
