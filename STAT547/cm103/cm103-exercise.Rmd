...
```{r}
# Method 1
map_chr(wesanderson, function(obj) obj[1])
# Method 2
map_chr(wesanderson, head, n = 1)
# Method 3
map_chr(wesanderson, `[`, 1) # actually calling function of the form `[`(x,1)
```
...
####### NOTE: use curly braces above, anything within curly braces only returns the last evaluation. So the pipe function will pipe got_chars not into tibble but the `{` function, which you can then use to make your tibble.
got_chars %>% 
    tibble(
        name = map_chr(., "name"),
        born = map_chr(., "born")
